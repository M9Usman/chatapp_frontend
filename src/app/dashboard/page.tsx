...
import { Send, Menu, X, LogOut, MessageSquare, Smile, Users, UserPlus, RefreshCcw, Bot } from "lucide-react"
...
export default function Dashboard() {
  ...
  const [chatSuggestionMessage,setChatSuggestionMessage]=useState(null);
  const [chatbotLoadFlag,setChatBotLoadFlag]=useState(false);
  const [chatbotMessages, setChatbotMessages] = useState([
    // Initial chatbot message (optional)
  ...
  useEffect(() => {
    console.warn("Messages Are Update: ", messages)
  }, [messages])
  ...
  const selectUserChat = async (user: User) => {
    try {
      ...
      if (
        responseUserChat?.data &&
        responseUserChat?.data?.messages?.chatId &&
        responseUserChat?.data?.messages &&
        responseUserChat?.data?.messages?.messages
      ) {
  ...
  const handleSearch = (query: string) => {
    ...
    const filtered = users.filter((user) => user.name.toLowerCase().includes(lowercasedQuery))
  ...
  const suggestMessageAi = async (message: any) => {
    ...
    try {
      ...
      scrollToBottom();
      console.log('SuggestionMessage Set : ',chatSuggestionMessage);
    } catch (error) {
      console.error("Failed to fetch suggested message:", error);
    }
  };
  ...
  const handleChatbotSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setChatBotLoadFlag(true);
    if (chatbotInput.trim() === "") return;

    const userMessage: any = { role: "user", content: chatbotInput };
    setChatbotMessages((prev) => [...prev, userMessage]);

    setChatbotInput("");

    try {
      // console.log('Chat Bot Conversation: ', chatbotInput,' auth token : ',authState.token);
      const botResponse = await getChatbotResponse(chatbotInput, authState.token);
      console.log('Bot response: ', botResponse);

      setTimeout(() => {
        
        setChatbotMessages((prev) => [...prev, botResponse]);
        setChatBotLoadFlag(false);
  ...
  return (
    <div className="flex h-screen bg-gray-950 text-white">
      <div
        className={`${isSidebarOpen ? "block" : "hidden"} md:block w-full md:w-64 flex-shrink-0 flex flex-col border-r border-gray-800 absolute md:relative z-10 bg-gray-950`}
      >
            ...
            groups?.map((group) => (
              <div className="flex flex-row p-2 justify-between items-center pr-4 ">
                ...
                >
                  ...
                  <p className="text-sm text-gray-400">{group.participants.length} participants</p>
                </div>
                <button
                  onClick={() => {
      ...
      <div className="flex-grow flex flex-col w-full">
        <div className="p-4 border-b border-gray-800 flex items-center">
          ...
          <h2 className="font-semibold">
            {groups.length > 0 ? selectedGroup?.name : selectedUser ? selectedUser.name : "Select a user to chat"}
          </h2>
        </div>
        ...
        {(selectedUser || selectedGroup) && (
          <div className="flex flex-col gap-1 p-1 pl-2">
            ...
            {chatSuggestionMessage && (
              <div className="flex flex-col gap-3 space-x-2 p-4 bg-black  transition rounded-lg shadow-md justify-starts items-start">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="ml-auto bg-blue-500 text-white hover:bg-blue-600 transition w-max p-4 text-lg"
                  onClick={()=>{    setMessageInput(chatSuggestionMessage!);
                  }}
                >
                  Use Suggestion
                </Button>
            ...
            <form onSubmit={handleSendMessage} className="p-4 border-t border-gray-800 flex items-center space-x-4">
              ...
              {selectedFile && (
                <Button
                  className="text-sm text-gray-200 p-2 rounded-md bg-slate-800 hover:bg-red-600 transition hover:cursor-pointer"
      ...
      {showCreateGroup && (
        <CreateGroupPopup users={users} onClose={() => setShowCreateGroup(false)} onCreateGroup={handleCreateGroup} />
      )}
      {/* Chatbot Button */}
      <Button
        variant="outline"
        size="icon"
        className="fixed top-4 right-4 z-50 rounded-full bg-blue-500 text-white hover:scale-125 hover:bg-white hover:shadow-white hover:text-blue-600 transition"
      ...
      {isChatbotOpen && (
        <div className="fixed top-16 right-4 w-80 h-96 bg-[#212121] rounded-lg shadow-lg flex flex-col">
          ...
          <ScrollArea className="flex-grow p-4">
            {chatbotMessages.map((msg, index) => (
              <div key={index} className={`mb-2 ${msg.role === "user" ? "text-right" : "text-left"}`}>
                <span className={`inline-block p-2 rounded-lg ${msg.role === "user" ? "bg-gray-100 text-black" : "bg-black"}`}>
                  {msg.content}
                </span>
              </div>
            ))}
            {chatbotLoadFlag && (
              <div className="flex flex-row justify-center items-center">
                <div className="gap-x-2 flex justify-center items-center bg-white p-2 rounded-full m-4 w-max animate-pulse">
                  <div className="w-3 h-3 rounded-full bg-blue-700 animate-bounce [animation-delay:.7s]"></div>
          ...
          <form onSubmit={handleChatbotSubmit} className="p-4 border-t border-gray-700">
