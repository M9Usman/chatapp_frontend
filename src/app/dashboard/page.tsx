              ...
              {messages.length === 0 ? (
                ...
                messages.map((message,index) => (
                  <div key={index} className="flex flex-col justify-center items-center">
                    <div className={`inline-block p-3 rounded-lg w-[85%] ${message.senderId === authState.user?.userId ? 'bg-[#5779D8]' : 'bg-[#0B183B]'}`}>
                      <p className="break-words whitespace-pre-wrap">{message.content}</p>
                      <p className="float-right text-xs text-gray-200 mt-3 rounded-full border border-gray-350 w-max p-1">{new Date(message.createdAt).toLocaleTimeString()}</p>
                    </div>
                  </div>
                ))
              )}
